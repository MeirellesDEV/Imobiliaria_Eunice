    ...
    <form class="background-blur" action="imoveis" method="post" id="painel-pesquisa-lateral" style="display:none">
        ...
        <div id="painel-pesquisa-opcoes-lateral">
            ...
            <input type="text" name="bairro" id="painel-local-lateral" class="input-text"
                placeholder="Pesquisar Localidade">
            <p>Endereço:</p>
            <input type="text" name="ruaNumero" id="painel-local-lateral" class="input-text"
                placeholder="Pesquisar Localidade">
            ...
            <div id="lista-produtos">
                @foreach ($imoveis as $item)
                    @if ($item->id != 0)
                        ...
                        <p class="produto-descricao"><span class="material-symbols-outlined">location_on</span> <span
                                class="produto-descricao-texto">{{ $item->cidade }}</span> </p>
                        <div class="produto-dados">

                            @if ($item->qtdDorms != 0)
                                <p id="dados-dorms"><span
                                        class="material-symbols-outlined">bed</span>{{ $item->qtdDorms }}</p>
                            @endif

                            @if ($item->qtdBanheiros != 0)
                                <p id="dados-banheiros"><span
                                        class="material-symbols-outlined">bathtub</span>{{ $item->qtdBanheiros }}</p>
                            @endif

                            @if ($item->qtdSuites != 0)
                                <p id="dados-suites"><span class="material-symbols-outlined">king_bed</span><span
                                        class="material-symbols-outlined">bathtub</span>{{ $item->qtdSuites }}</p>
                            @endif

                            @if ($item->qtdGaragemCobertas != 0)
                                <p id="dados-vagas"><span class="material-symbols-outlined">garage</span><span
                                        class="material-symbols-outlined">garage_home</span>{{ $item->qtdGaragemCobertas }}
                                </p>
                            @endif

                            @if ($item->qtdGaragemNaoCobertas != 0)
                                <p id="dados-vagasDescobertas"><span
                                        class="material-symbols-outlined">garage</span>{{ $item->qtdGaragemNaoCobertas }}
                                </p>
                            @endif
